...
{
	int capacity = 3; // 현재 배열의 최대 용량 (처음엔 방 3개)
	int count = 0;    // 현재 실제로 저장된 데이터 개수
	int input;
	...
	while (true) {
		...
		// 준비한 메모리 다 사용했을 경우 크기를 2배로 확장
		if (count == capacity) {
			capacity *= 2; // 용량을 2배로 타겟팅
			printf("메모리 부족하여 배열 크기를 %d개로 확장!\n", capacity);

			// realloc으로 기존 데이터를 보존하면서 배열 확장
			...
			arr = temp; // 새롭게 확장된 주소로 갱신
		}

		arr[count] = input; // 데이터 저장
		count++;
	}
